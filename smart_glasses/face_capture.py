...
class FaceRecognitionApp:
    def __init__(self, existing_camera=None):
        # Initialize running state FIRST, before any other operations
        self.running = False
        ...
        # Initialize face recognition data structures
    ...
    def run_face_recognition(self):
        ...
        self.speak("Starting face recognition. Say 'stop' or press 'q' to stop.")
        print("Looking for faces... Say 'stop' or press 'q' to stop.")
        last_spoken_names = set()
        last_voice_check = time.time()
        voice_check_interval = 1.0  # Check for voice command every second

        while recognition_active and self.running:
            try:
                ...
                # Check for voice command 'stop' periodically
                current_time = time.time()
                if current_time - last_voice_check >= voice_check_interval and self.mic_found:
                    ...
                    if command:
                        if 'stop' in command.lower():
                            ...
                            # Process other commands without breaking the loop
                ...
                if key == ord('q'):
                    ...
                    if self.mic_found:
                        ...
                        if response and "yes" in response.lower():
                            recognition_active = False  # Only stop recognition temporarily
                            self.add_face()
                            recognition_active = True   # Resume after adding face
                    else:
        ...
        cv2.waitKey(1)  # Additional waitKey to ensure window is destroyed
    ...
    def add_face(self):
        ...
        self.speak("Looking for a clear face to add. Please stay still when a face is detected.")
        print("Capturing image for new face... Press 'q' to cancel.")
        ...
        while not face_found:
            try:
                ...
                cv2.imshow('Add Face - Position yourself in frame', frame)

                # Handle keyboard input
                key = cv2.waitKey(1) & 0xFF
                if key == ord('q'):
                    cv2.destroyAllWindows()
                    self.speak("Face addition cancelled.")
                    return

                if len(face_locations) == 1:  # Exactly one face detected
                    if not face_found:  # First time detecting a face
                        self.speak("Face detected! Hold still...")
                        time.sleep(1)  # Give a moment to ensure stable position
                        
                        # Get the encoding
                        face_encoding = face_recognition.face_encodings(rgb_frame, face_locations)[0]

                        # Check for similarity with existing faces
                        if len(self.known_face_encodings) > 0:
                    ...
                    if not face_found:
                        self.speak("Multiple faces detected. Please ensure only one person is visible.")
                    time.sleep(1)
    ...
    def stop_recognition(self):
        """Stop face recognition mode"""
        self.running = False
        print("Stopping face recognition mode...")

    def process_command(self, command):
        """Process voice commands during face recognition"""
    ...
    def run(self):
        """Main loop for face recognition with voice commands."""
        try:
            self.running = True  # Set running state at the start
            self.speak("Face recognition mode active. Say 'stop' or press 'q' to exit.")
            
            # Single main loop
            self.run_face_recognition()
            
            cv2.destroyAllWindows()
            self.speak("Exiting face recognition mode.")

        except KeyboardInterrupt:
            self.speak("Face recognition stopped.")
        except Exception as e:
            print(f"Error in face recognition: {e}")
            self.speak("Sorry, there was an error in face recognition.")
        finally:
            cv2.destroyAllWindows()
            self.running = False  # Ensure running is set to False on exit
